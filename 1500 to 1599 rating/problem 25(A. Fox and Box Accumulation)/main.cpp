...
void solve() {
  ...
  sort(x.begin(), x.end()); // sort strengths

  vector<int> piles; // keeps count of how many boxes are in each pile

  for (int i = 0; i < n; i++) {
    bool placed = false;
    for (int j = 0; j < (int)piles.size(); j++) {
      if (piles[j] <= x[i]) { // can put box i on pile j
        piles[j]++;
        placed = true;
        break;
      }
    ...
    if (!placed) {
      piles.push_back(1); // start new pile
    }
  }
